...
// are now defined in chat_popup_window.php and should NOT be redefined here
    ...
    // Generate media HTML (image or video) - FIXED VERSION
    let mediaHTML = '';
    if (recipe.image_url || recipe.video_url) {
        mediaHTML = '<div class="recipe-media">';
        if (recipe.image_url) {
            mediaHTML += `<img src="${recipe.image_url}" alt="${recipe.title}" class="recipe-image" onerror="this.style.display='none'">`;
        } else if (recipe.video_url) {
            mediaHTML += `
